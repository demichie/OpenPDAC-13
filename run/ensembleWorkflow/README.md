...
1.  **Configure Script 1**: Open the sampling script and carefully define all your parameters in the `parameter_config` list.
...
3.  **Run Script 1**: Execute the sampling script.
    ```bash
    python sampling_script.py
    ```
4.  **Review Output**: Check the generated `samples.csv` to ensure the values are reasonable. Examine the plot files (`.png`, `.pdf`) to visually confirm the distributions and correlations.
5.  **Run Script 2**: Execute the ensemble generation script.
    ```bash
    python ensemble_generator.py
    ```
6.  **Verify Ensemble**: Inspect a few of the generated `ensemble.XXXXX` directories to confirm that the files have been copied and the placeholders have been correctly replaced.
7.  You are now ready to run your simulations.
